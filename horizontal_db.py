...
def select_query(dictionary):
    query = sql.SQL('SELECT * FROM public.retro_horizontal WHERE "AUDITORIA" = %s AND "RODOVIA" = %s AND "KM_I"=%s AND "METRO_I"=%s AND "DATA"= %s AND "SENTIDO"=%s' )
    values = (dictionary['AUDITORIA'], dictionary['RODOVIA'], str(dictionary['KM_I']), str(dictionary['METRO_I']), str(dictionary['DATA']), str(dictionary['SENTIDO']))
    return query, values
